...
function githubApiRequest(method, apiPath, body) {
  ...
  const path = apiPath.startsWith('/repos/') ? apiPath : `/repos/${owner}/${repo}${apiPath}`
...
async function getGithubFileSha(filePath) {
  try {
    const { data } = await githubApiRequest('GET', `/contents/${encodeURIComponent(filePath)}?ref=${encodeURIComponent(GH_REF)}`)
    return data.sha
  } catch (e) {
...
async function uploadGithubFile(filePath, base64Content, message) {
  ...
  const { data } = await githubApiRequest('PUT', `/contents/${encodeURIComponent(filePath)}`, body)
  return data
}
